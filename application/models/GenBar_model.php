...
class Genbar_Model extends CI_model
{
	...
	public function getshow_query($id_karyawan)
	{
		...
		$this->db->select('k.id_karyawan, k.nama_karyawan, j.nama_jabatan, s.alamat');
		$this->db->from('karyawan AS k');
		$this->db->join('jabatan AS j', 'j.id_jabatan = k.jabatan');
		$this->db->join('gedung AS s', 's.gedung_id = k.gedung_id');
		$this->db->where('k.nama_karyawan', $_POST['id']);
